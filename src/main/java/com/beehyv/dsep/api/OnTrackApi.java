...
@Validated
@Tag(name = "on_track", description = "the on_track API")
@RequestMapping("${openapi.becknCore.base-path:}")
public interface OnTrackApi {

    default OnTrackApiDelegate getDelegate() {
        return new OnTrackApiDelegate() {};
    }
    ...
    default ResponseEntity<SearchPost200Response> onTrackPost(
        @Parameter(name = "OnTrackPostRequest", description = "TODO") @Valid @RequestBody(required = false) OnTrackPostRequest onTrackPostRequest
    ) {
        return getDelegate().onTrackPost(onTrackPostRequest);
    }
